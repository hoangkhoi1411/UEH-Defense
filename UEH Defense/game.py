...
#Name and Icon
name = pygame.display.set_caption("UEH Defense")
icon = pygame.image.load('spaceship.png')
pygame.display.set_icon(icon)

#Background
background = pygame.image.load('Terrain/background.png')
game_menu = pygame.image.load('Terrain/gamemenu.png')
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, WIDTH-300), random.randrange(-1000, -100), random.choice(["Enemy1", "Enemy2", "Enemy3" , "Enemy4" , "Enemy5" , "Enemy6" , "Enemy7"]))
...
#Game Loop
def main_menu():
    name_font = pygame.font.SysFont("04B_19.ttf", 60)
    run = True
    blink_timer = 0
    background_music.play(-1)
    while run:
        ...
        #Flashing Texts
        if blink_timer < 30:
            name_label = name_font.render("Click to start...", 1, (255,255,255))
            screen.blit(name_label, (WIDTH/2 - name_label.get_width()/2, 490))
        elif blink_timer < 60:
            pass
        else: blink_timer = 0
        blink_timer += 1
